...
class _HumidityState extends State<Humidity> {
  ...
  void _getActivityData(String activity){
    ...
    //This child has to be the connected user ID
      ...
      min = 500;
      userData.forEach((key, value) {
        if (int.parse(value['humidity']) > max) {
          max = int.parse(value['humidity']);
        }
        if (int.parse(value['humidity']) < min) {
          min = int.parse(value['humidity']);
        }
        ;
      });
      setState(() {
                  ...
                  child:SfCartesianChart(
                    ...
                    series: <ChartSeries>[
                      ColumnSeries<myData, DateTime>(
                          ...
                          yValueMapper: (myData dataRow, _) => double.parse(dataRow.humidity)
                      )
                    ],
